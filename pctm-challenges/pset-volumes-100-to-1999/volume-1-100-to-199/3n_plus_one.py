...
print n
if n = 1 then STOP
if n is odd then n ←− 3n + 1 ODD
else n ←− n/2 GOTO 2  EVEN
...
@memoize({(1,): 1})
def determine_cycling_length(num):
    cycle_length = 1
    while num is not 1:
        if num % 2 == 0:
            num = num // 2
...
def find_max_cycling_length(i, j):
    cycling_lengths = []
    for num in range (i, j + 1):
        cycling_lengths.append(determine_cycling_length(num))
    print(i, j, max(cycling_lengths))

def main():
    for line in sys.stdin:
        min_num, max_num = int(line.split()[0]), int(line.split()[1])
        assert 0 < min_num < 1000000 and 0 < max_num < 1000000
        find_max_cycling_length(min_num, max_num)
    sys.exit(0)
